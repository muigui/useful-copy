...
import {copy, is_plain_object, merge, update} from "./index";
...
suite('muigui/useful-copy', function () {
    ...
    test('<static> merge', function (done) {
        var expected    = {
                foo : 'bar',
                items : [{value : 1}, {
                    items : [{value : 1}, {
         ...
         var returned    = merge(Object.create(null), expected);
         var overwritten = merge({items : [{value : '1 2 3'}, {items : Array.apply([], new Array(100)).map(function (o, i) { return i + 1; })}]}, expected);
    ...
    test('<static> update', function (done) {
        var expected    = {
                foo : 'bar',
                items : [{id : 1, value : 1}, {
                    items : [{value : 1}, {
         ...
         var returned    = update(Object.create(null), expected);
         var overwritten = update({
                foo : 'foo',
                items : [{value : '1 2 3'}, {items : [{id : 0}, {items : [{id : 2}]}].concat([0, 1, 2, 3])}]
            }, expected);
